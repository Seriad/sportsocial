...
public interface ClubRepository extends CrudRepository<Club, Long>{
	...
	@Query("SELECT ca FROM Club c INNER JOIN c.admin ca")
	List<User> getAdmin(Long idClub);

	@Query("SELECT cm FROM Club c INNER JOIN c.membres cm WHERE cm.idUser NOT IN (SELECT ca.idUser FROM Club c INNER JOIN c.admin ca)")
